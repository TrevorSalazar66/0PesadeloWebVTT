...
export const dbQueries = {
  ...
  async ensureScenesTable(db) {
    try {
      ...
      console.warn('Tabela scenes ja existe ou erro ao criar:', e);
    }

    // Migrações dinâmicas preventivas para cada coluna caso a tabela já existisse em versão anterior
    const cols = [
      "ALTER TABLE scenes ADD COLUMN model TEXT DEFAULT 'tactical_grid'",
      "ALTER TABLE scenes ADD COLUMN model_data TEXT DEFAULT '{}'",
      "ALTER TABLE scenes ADD COLUMN rules_data TEXT DEFAULT '[]'",
      "ALTER TABLE scenes ADD COLUMN style_data TEXT DEFAULT '{}'",
      "ALTER TABLE scenes ADD COLUMN state_data TEXT DEFAULT '{}'",
      "ALTER TABLE scenes ADD COLUMN max_players INTEGER DEFAULT 12",
      "ALTER TABLE scenes ADD COLUMN is_active INTEGER DEFAULT 0",
      "ALTER TABLE scenes ADD COLUMN xp_triggers TEXT DEFAULT '[]'",
      "ALTER TABLE scenes ADD COLUMN updated_at TEXT DEFAULT CURRENT_TIMESTAMP"
    ];
    for (const sql of cols) {
      try {
        await db.prepare(sql).run();
      } catch (_) {
        // Ignora erro se coluna já existir no SQLite
      }
    }
  },
  ...
  async createScene(db, { id, campaignId, name, description = '', imageUrl = '', model = 'tactical_grid', modelData = {}, rulesData = [], styleData = {}, stateData = {}, maxPlayers = 12, xpTriggers = [], isActive = 0 }) {
    ...
    try {
      ...
      // Se falhar por qualquer motivo de schema, tenta rodar a migração novamente
      await this.ensureScenesTable(db);
      const stmtRetry = db.prepare(`
        INSERT INTO scenes (id, campaign_id, name, description, image_url, model, model_data, rules_data, style_data, state_data, max_players, xp_triggers, is_active, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
      `);
      return await stmtRetry.bind(
        id,
        campaignId,
        name,
        description || '',
